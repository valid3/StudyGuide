...
class MAIN(customtkinter.CTk):
    def __init__(self):
        ...
        # Frames Pre-Setup
        self.GuidesMain = customtkinter.CTkFrame(
            master=self,
            width=240,
            height=540,
        ...
        self.InformationMain = customtkinter.CTkFrame(
            master=self,
            width=596,
            height=60,
            border_width=self.BorderWidth,
        ...
        self.AnsweringMain = customtkinter.CTkFrame(
            master=self,
            width=596,
            height=411,
            border_width=self.BorderWidth,
        ...
        self.GuidesScrollFrame = customtkinter.CTkScrollableFrame(
            master=self.GuidesMain,
            width=215,
            height=528,
            corner_radius=3,
            fg_color="#2B2B2B",
        ...
        self.OutputMain = customtkinter.CTkFrame(
            master=self.AnsweringMain,
            width=580,
            height=220,
            border_width=self.BorderWidth,
            border_color=self.BorderColor,
        )
        self.OutputScrollFrame = customtkinter.CTkScrollableFrame(
            master=self.OutputMain,
            width=555,
            height=208,
            corner_radius=3,
            fg_color="#2B2B2B",
        ...
        self.ProblemsLeft = customtkinter.CTkLabel(
            master=self.InformationMain,
            width=285,
            height=54,
            font=("Arial Baltic", 25),
            text="Problems: None",
            anchor="e",
        )
        self.LoadedGuideName = customtkinter.CTkLabel(
            master=self.InformationMain,
            width=285,
            height=54,
            font=("Arial Baltic", 25),
        ...
        self.QuestionDisplayText = customtkinter.CTkLabel(
            master=self.AnsweringMain,
            width=578,
            height=65,
            corner_radius=3,
        ...
        self.AnswerEntry = customtkinter.CTkEntry(
            master=self.AnsweringMain,
            width=385,
            height=35,
        ...
        self.ClearLogButton = customtkinter.CTkButton(
        ...
        # Switches
        self.LogWrong = customtkinter.CTkSwitch(
            master=self.OptionsMain,
            text="Log Wrong",
            height=49,
            font=("Arial", 16),
            button_color="#222222",
            progress_color="#717171",
            button_hover_color="#171717",
        )
        self.LogMisspellings = customtkinter.CTkSwitch(
            master=self.OptionsMain,
            text="Log Misspellings",
            height=49,
            font=("Arial", 16),
            button_color="#222222",
            progress_color="#717171",
        ...
        for (
            ...
            self.guides.items()
        ):  # Loops through guides folder and making loader for each in GuidesScrollFrame
            # Frame/Labels/Button Pre-Setup
            self.GuideFrame = customtkinter.CTkFrame(
                master=self.GuidesScrollFrame, width=210, height=61, fg_color="#242424"
            )
            self.GuideNameDisplay = customtkinter.CTkLabel(
            ...
            self.GuideProblems = customtkinter.CTkLabel(
                master=self.GuideFrame,
            ...
            self.GuideLoadButton = customtkinter.CTkButton(
                master=self.GuideFrame,
                width=54,
                height=54,
                text="Load",
            ...
            self.GuideLoadButton.configure(
                command=lambda name=GuideName, answers=GuideAnswers, asked=False: self.LoadGuide(
                    name, answers, asked
                )
            )
    ...
    def Process(
        Answer,
    ):  # Enhances Quality of Life by making answering questions less tedious
        if type(Answer) != int:
    ...
    def Exit(self):  # Asks user if they want to exit program
        if tkinter.messagebox.askyesno(
            "Exit",
            "Are you sure you want to exit? Your submitted answers will not save for the next time when you open.",
        ):
            self.destroy()
    ...
    def ShowMisspelling(self, word, misspelling):  # Logs the Misspelling
        if self.LogMisspellings.get() == 1:
            # Pre-Setup
            LogFrame = customtkinter.CTkFrame(
                master=self.OutputScrollFrame, width=549, height=46, fg_color="#242424"
            )
            WordLabel = customtkinter.CTkLabel(
                ...
                font=("Arial", 16),
                text_color="yellow",
                text=f"Misspelled Word: {misspelling}",
            )
            CorrectButton = customtkinter.CTkButton(
                master=LogFrame, width=100, height=40, text="Correct Spelling"
            )

            def SHOW():  # Shows the misspelled word in an info box
                tkinter.messagebox.showinfo(
                    "Correct Spelling",
                    f"You spelt the word as '{misspelling}', the correct way of spelling is '{word}'.",
                )
    ...
    def ShowRightAnswer(self, question, answer):  # Logs the Misspelling
        if self.LogWrong.get() == 1:
            ...
            QuestionLabel = customtkinter.CTkLabel(
                master=LogFrame,
                width=100,
                height=40,
                font=("Arial", 16),
            ...
            ShowAnswer = customtkinter.CTkButton(
                master=LogFrame, width=100, height=40, text="Correct Answer"
            )
    ...
    def Restart(self):  # Restarts Current Loaded Guide
        if self.loaded:
            if tkinter.messagebox.askyesno(
                "Load",
                f"Are you sure you want to restart '{self.LoadedGuideName.cget('text')[:4]}'?",
            ):
                self.Finalize()
    ...
    def SubmitAnswer(self):  # Submit an answer to current question
        if not self.submitted:
            if self.loaded:
                ...
                if (
                    self.problems[CurrentQuestion]["MISSPELLINGS"] is not None
                    and self.problems[CurrentQuestion]["MISSPELLINGS"]
                ):  # Checks if there are any misspellings available
                    for word, misspellings in self.problems[CurrentQuestion][
                        "MISSPELLINGS"
                    ].items():
                        for misspelling in misspellings:
                            if misspelling in UserAnswer:
                                ...
                                self.found = True
                                self.QuestionDisplayText.configure(
                                    text="Correct! Be sure to check spelling! Press 'Submit' again for the next question!",
                                    text_color="yellow",
                                )
                ...
                if not self.found:  # This check is for if a misspelling was found
                    if all(
                        word in UserAnswer
                        for word in self.problems[CurrentQuestion]["ANSWERS"]
                    ):  # Checks answer
                        self.QuestionDisplayText.configure(
                            text="Correct! Press 'Submit' again for the next question!",
                            text_color="green",
                        )
                        self.problems.pop(CurrentQuestion)
                    else:  # This fires when the user answered incorrectly
                        self.QuestionDisplayText.configure(
                            text=f"Wrong! The answer is {' '.join(self.problems[CurrentQuestion]['ANSWERS'])}! Press 'Submit' again for the next question!",
                            text_color="red",
                        )
                        self.ShowRightAnswer(
                            CurrentQuestion,
                            " ".join(self.problems[CurrentQuestion]["ANSWERS"]),
                        )
                        self.problems.pop(CurrentQuestion)
                        self.grade -= 1
            ...
            if self.problems:  # If there are problems, give next problem
                ...
                Result = tkinter.messagebox.askyesno(
                    "Results",
                    f"You scored {(100/self.problemCount)*self.grade}% ({self.grade}/{self.problemCount}). Would you like to restart?",
                )
                if Result:  # If user presses Yes to restart, this fires
                    self.Finalize()
                    self.LoadGuide(
                        self.LoadedGuideName.cget("text")[:4], self.copy, True
                    )
    ...
    def GiveProblem(self):  # Gives randomized problem to user
        self.ProblemsLeft.configure(text=f"Problems: {len(self.problems)}")
        self.QuestionDisplayText.configure(
            text=random.choice(list(self.problems.keys()))
        )
    ...
    def LoadGuide(self, name, answers, asked):  # Loads a Guide
        def LOAD():  # Quality of Life, makes it so if you press Yes on restart button, you won't need to press another button
            if not self.loaded:  # If guide not already loaded
                if (
                    self.copy
                ):  # Made for handling restarts, checks to see if copy exists, and if it does that's program way of knowing a restart has been initiated
                    self.problems = self.copy.copy()

                self.LoadedGuideName.configure(
                    text=f"Name: {name}"
                )  # Changes loaded guide name to name of the guide that was loaded
                self.problems = (
                    answers.copy()
                )  # Shallow copies input (answers) into self.problems
                self.copy = self.problems.copy()

                for Question, Answer in self.problems.copy().items():
                    self.problems[Question] = {"ANSWERS": Answer}
                    self.problems[Question]["ANSWERS"] = self.Process(
                        self.problems[Question]["ANSWERS"]
                    )
                ...
                if self.CanHaveMisspelling():
                    for QuestionMIS, AnswerMIS in self.problems.items():
                        for Word in AnswerMIS["ANSWERS"]:
                            if len(Word) >= 4:
                                self.problems[QuestionMIS]["MISSPELLINGS"][
                                    Word
                                ] = self.ProcessMisspellings(Word)
                else:
